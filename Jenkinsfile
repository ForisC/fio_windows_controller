...
node {
    withCredentials([sshUserPrivateKey(credentialsId: 'ubuntu_clinet', keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'userName'), passwordVariable: 'PASSWORD']) {
        ...
        stage("FIO random read") {
            command = "fio -filename=${env.drive}:\\fio -direct=1 -iodepth 1 -thread -rw=randread -ioengine=windowsaio -bs=16k -size=${env.TestSize} -numjobs=30 -runtime=1000 -group_reporting -name=randread --output=randread.log --output-format=json"
